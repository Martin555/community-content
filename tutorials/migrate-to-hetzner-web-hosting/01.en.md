...
| password                 | The password for the mailbox - if the field is empty you can set the password in konsoleH after the import. Please follow the password guidelines:<br /> The password must be at least 12 characters long and contain only allowed characters:<li>At least one lowercase letter: `a-z`</li><li>At least one uppercase letter: `A-Z`</li><li>At least one number: `0-9`</li><li>At least one allowed special character: `^ ! $ % / ( ) = ? + - # . , ; : ~ * @ { } _ & [ ]`</li><li>No umlauts</li> |
